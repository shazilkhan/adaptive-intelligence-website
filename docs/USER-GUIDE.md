...
### Frontend (Amplify / `.env.local`)

| Variable | Where | Purpose |
|----------|--------|--------|
| `NEXT_PUBLIC_STRAPI_API_URL` | Amplify env vars (and in `amplify.yml` for build) | Base URL of Strapi, e.g. `https://admin.adaptiveintelligence.online`. No trailing slash. |
...
### Backend (App Runner / `backend-new/.env` or `.env.apprunner`)
